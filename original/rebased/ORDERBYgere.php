		...
		<br /> 
            <table> <!--Copyright Amine-->
                <tr> <!-- table row = colonnes-->
                    ...
                    $query = 'SELECT id, salle, nom, gere, quantite, prix FROM bibliographie ORDER BY gere'; //AJOUT DE ORDER BY ID ASC
                    // Lance la requête SQL  copyright Amine
                    $result = $rt->query($query) or die('Erreur SQL !<br>'.$query.'<br>'.mysqli_error()) ;                    
                    // On parcourt la liste des enregistrements Copyright Amine
                    while ($row = mysqli_fetch_assoc($result)) { 
                    // htmlentities convertie les caractères accentués pour qu’ils s’affichent correctement en HTML 
                        $gere = htmlentities($row['gere']); //copyright Amine

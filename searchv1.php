		...
		<br /> 
            <table> 
                <tr> <!-- table row = colonnes-->
                    ...
                    <?php 
                if(isset($_GET['recsearch'])) { 
                    $recsearch=addslashes($_GET['recsearch']);
                    echo($recsearch);	
                    ...
                    $query = 'SELECT id, salle, nom, gere, quantite, prix FROM bibliographie where id like '$recsearch'  '; //AJOUT DE ORDER BY ID ASC
                    // $query = 'SELECT id, salle, nom, gere, quantite, prix FROM bibliographie where id='$recsearch' OR salle='$recsearch' OR nom='$recsearch' or gere='$recsearch' or quantite='$recsearch' or prix='$recsearch''; //AJOUT DE ORDER BY ID ASC
                    // $query = 'SELECT id, salle, nom, gere, quantite, prix FROM bibliographie where id=$recsearch or salle=$recsearch or nom=$recsearch or gere=$recsearch or quantite=$recsearch or prix=$recsearch'; //AJOUT DE ORDER BY ID ASC
